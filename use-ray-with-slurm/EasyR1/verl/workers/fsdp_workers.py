...
class FSDPWorker(Worker):
    ...
    def _init_dist_mesh(self, config: Union[ActorConfig, CriticConfig], role: Literal["actor", "critic"]):
        ...
        # Debug logging to understand the configuration values
        self.print_rank0(f"{role} FSDP config - enable_cpu_offload: {config.fsdp.enable_cpu_offload}")
        self.print_rank0(f"{role} Batch sizes - global_batch_size_per_device: {config.global_batch_size_per_device}, micro_batch_size_per_device_for_update: {config.micro_batch_size_per_device_for_update}")
        
        # Validate CPU offload configuration
        if config.fsdp.enable_cpu_offload:
            if config.global_batch_size_per_device != config.micro_batch_size_per_device_for_update:
                raise ValueError(f"{role} cannot use FSDP's CPU offload when gradient accumulation is enabled.")
            else:
                self.print_rank0(f"{role} CPU offload is enabled and no gradient accumulation detected - validation passed.")
        else:
            self.print_rank0(f"{role} CPU offload is disabled - skipping validation.")

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def init_fsdp(self):
        """Initialize FSDP before model loading to ensure proper memory management"""
        print(f"DEBUG: Initializing FSDP for {self.role} worker")
        
        # Initialize FSDP for actor if present
        if self._has_actor:
            print(f"DEBUG: Initializing FSDP for actor in {self.role}")
            self._init_fsdp_for_role("actor")
        
        # Initialize FSDP for critic if present
        if self._has_critic:
            print(f"DEBUG: Initializing FSDP for critic in {self.role}")
            self._init_fsdp_for_role("critic")
        
        # Initialize FSDP for ref if present
        if self._has_ref:
            print(f"DEBUG: Initializing FSDP for ref in {self.role}")
            self._init_fsdp_for_role("ref")
        
        print(f"DEBUG: FSDP initialization completed for {self.role} worker")

    def _init_fsdp_for_role(self, role: str):
        """Helper method to initialize FSDP for a specific role"""
        try:
            if role == "actor":
            ...
            # Initialize FSDP with empty weights first
            print(f"DEBUG: Creating FSDP module for {role}")
            
            # Create a minimal FSDP structure without loading the full model
            # This ensures FSDP is properly initialized before model loading
            with init_empty_weights():
                if hasattr(self, f"{role}_fsdp_module"):
                    # FSDP module already exists, just ensure it's properly initialized
                    print(f"DEBUG: FSDP module for {role} already exists")
                else:
                    print(f"DEBUG: FSDP module for {role} will be created during model loading")
            
            print(f"DEBUG: FSDP initialization for {role} completed")
            
        except Exception as e:
            print(f"WARNING: Failed to initialize FSDP for {role}: {e}")
