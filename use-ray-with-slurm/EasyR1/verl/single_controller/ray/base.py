...
class RayResourcePool(ResourcePool):
    ...
    def get_placement_groups(self, strategy: str = "STRICT_PACK", name: Optional[str] = None) -> List[PlacementGroup]:
        ...
        pg_name_prefix = (
            name if name else f"{self.name_prefix}verl_group_{'_'.join([str(count) for count in self._store])}_{unique_id}:"
        )
        ...
        # Clean up any existing placement groups with similar names to prevent conflicts
        try:
            import ray
            existing_pgs = ray.util.list_placement_groups()
            for pg in existing_pgs:
                if pg.name and pg.name.startswith(self.name_prefix + "verl_group_"):
                    try:
...
class RayWorkerGroup(WorkerGroup):
    ...
    def init_fsdp(self) -> None:
        """Initialize FSDP for all workers in this group before model loading"""
        print(f"DEBUG: Initializing FSDP for {len(self._workers)} workers in group {self.name_prefix}")
        for i, worker in enumerate(self._workers):
            try:
                print(f"DEBUG: Initializing FSDP for worker {i}")
                # Call init_fsdp on each worker
                ray.get(worker.init_fsdp.remote())
                print(f"DEBUG: FSDP initialized for worker {i}")
            except Exception as e:
                print(f"WARNING: Failed to initialize FSDP for worker {i}: {e}")
                # Continue with other workers
        print(f"DEBUG: FSDP initialization completed for group {self.name_prefix}")
    ...
    def _init_with_resource_pool(
        ...
        print(f"DEBUG: GPUs per worker: {num_gpus}")

        rank = -1
        local_world_size = resource_pool.store[0]
        print(f"DEBUG: Local world size: {local_world_size}")
        
        for pg_idx, pg in enumerate(sort_placement_group_by_node_ip(pgs)):
            print(f"DEBUG: Processing placement group {pg_idx}: {pg}")
            assert local_world_size <= pg.bundle_count, f"when generating for {self.name_prefix}, for the "
            for local_rank in range(local_world_size):
                rank += 1
                print(f"DEBUG: Creating worker with rank {rank}, local_rank {local_rank}")
                ...
                # create a worker with timeout
                print(f"DEBUG: Creating worker {name}...")
                try:
                    import signal
                    
                    def timeout_handler(signum, frame):
                        raise TimeoutError(f"Worker creation timed out after 60 seconds for {name}")
                    ...
                    # Cancel the alarm
                    signal.alarm(0)
                    print(f"DEBUG: Worker {name} created successfully")
                    
                except TimeoutError as e:
                    print(f"ERROR: Worker creation timed out: {e}")
                    # Clean up and retry once
                    try:
                        print("DEBUG: Attempting to clean up and retry...")
                        # Force cleanup of placement groups
                        ray.util.placement_group.remove(pg)
                        time.sleep(5)
                        
                        # Retry without placement group constraint
                        print("DEBUG: Retrying without placement group constraint...")
                        worker = ray_cls_with_init(
                            placement_group=None, placement_group_bundle_idx=0, use_gpu=use_gpu, num_gpus=num_gpus
                        )
                        print(f"DEBUG: Worker {name} created successfully on retry")
                    except Exception as retry_e:
                        print(f"ERROR: Retry failed: {retry_e}")
                        raise RuntimeError(f"Failed to create worker {name} after timeout and retry: {retry_e}")
                        
                except Exception as e:
                    print(f"ERROR: Failed to create worker {name}: {e}")
                    raise
                ...
                if rank == 0:
                    print("DEBUG: Waiting for register center actor...")
                    ...
                    print(f"DEBUG: Got master info: {self._master_addr}:{self._master_port}")
